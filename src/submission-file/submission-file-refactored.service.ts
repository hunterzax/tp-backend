...
export class SubmissionFileRefactoredService {
  ...
  ckDateInfoNomDailyAndWeeklyNew( 
    ...
    if (startDateExConv.isBefore(allowedDate, 'day')) {
      ...
      // เกินวัน submission ให้เช็ค renom
      if (!nominationDeadlineReceptionOfRenomination) {
        ...
        const renomAllowedDate = nowAts
          ...
          .startOf('minute');
        console.log('renomAllowedDate : ', renomAllowedDate);
        if (startDateExConv.isBefore(renomAllowedDate, 'day')) {
          console.log('เกิน renom deadline');
          throw new HttpException(
            {
              status: HttpStatus.BAD_REQUEST,
              error: 'Start Date is over submission deadline.', // https://app.clickup.com/t/86etzcgxn
          ...
          if (startDateExConv.isSame(renomAllowedDate, 'day')) {
            console.log('วันตรงกัน renom');
            if (renomAllowedDate.isSameOrBefore(nowAts)) {
              console.log('ยังไม่เกิน renom ผ่านได้ _');
              return true;
            } else {
      ...
      if (startDateExConv.isSame(allowedDate, 'day')) {
        ...
        if (allowedDate.isBefore(nowAts)) {
          ...
          if (!nominationDeadlineReceptionOfRenomination) {
            ...
            const renomAllowedDate = nowAts
              ...
              .startOf('minute');
            console.log('renomAllowedDate : ', renomAllowedDate);
            if (renomAllowedDate.isBefore(nowAts, 'day')) {
              console.log('เกิน renom deadline');
              throw new HttpException(
                {
                  status: HttpStatus.BAD_REQUEST,
              ...
              console.log('ภายใน renom deadline');
              if (renomAllowedDate.isSame(nowAts, 'day')) {
                console.log('วันตรงกัน renom');
                if (renomAllowedDate.isSameOrBefore(nowAts)) {
                  console.log('ยังไม่เกิน renom ผ่านได้ _');
                  return true;
  ...
  async uploadFile(
    ...
    const { todayStart, todayEnd, nowAts, gAuserType } =
      await this.initialSetupService.executeInitialSetup(userId);

    // ===== STEP 2: FILE TYPE VALIDATION =====
    const { findData, checkType, nomination_type_id } =
      await this.fileTypeValidationService.executeFileTypeValidation(
        file,
        tabType,
      );
    ...
    let {
      ...
      contract_code_id,
      startDateEx,
    } = await this.sheetDataExtractionService.executeSheetDataExtraction(
      findData,
      checkType,
    );

    // ===== STEP 7-11: STATUS AND PERMISSION VALIDATION =====
    await this.statusValidationService.executeStatusValidation(
      shipper_id,
      shipperCompare,
      contract_code_id,
      contractCodeNameCompare,
    ...
    } = await this.templateValidationService.executeTemplateValidation(
      shipper_id,
      contract_code_id,
      nomination_type_id,
      gAuserType,
    ...
    // เช็คถึงตรงนี้ ข้างล่างยังไม่ได้เช็ค.......
    ...
    let {
      ...
      conceptPoint,
      // isEqualSheet2
    } = await this.dataProcessingService.executeDataProcessing(
      startDateEx,
      todayStart,
      todayEnd,
      sheet2,
               ...
               if (
                ...
                if (finds) {
                  warningLogDayWeekTemp = warningLogDayWeekTemp?.map((ed: any) => {
                    const neD = ed;
                    if (
                      finds?.headDayUse === neD?.headDayUse &&
                      finds?.contractPoint === neD?.contractPoint &&
                      finds?.nomination_point === ed?.nomination_point
                    ) {
    ...
    for (let ig = 0; ig < groupedBywarningLogHrTemp.length; ig++) {
      ...
      if (Number(energyValues) > Number(groupedBywarningLogHrTemp[ig]?.value)) {
        warningLogHr.push(
          `Nominated max energy ${this.formatNumberThreeDecimal(energyValues)} exceeds contracted value ${this.formatNumberThreeDecimal(groupedBywarningLogHrTemp[ig]?.value) || ''} for contract point ${groupedBywarningLogHrTemp[ig]?.contractPoint || '-'} and hour ${groupedBywarningLogHrTemp[ig]?.hr || '-'}`,
        );
      }
    }
    ...
    for (let ig = 0; ig < groupedBywarningLogTotalTemp.length; ig++) {
      ...
      if (
        Number(energyValues) > Number(groupedBywarningLogTotalTemp[ig]?.valueDay)
      ) {
        console.log('***');
        console.log('Number(energyValues) : ', Number(energyValues));
        console.log(Number(groupedBywarningLogTotalTemp[ig]?.valueDay));
        warningLogDay.push(
          `Nominated Total energy ${(energyValues && this.formatNumberThreeDecimal(parseToNumber(energyValues))) || 0} exceeds contracted value ${this.formatNumberThreeDecimal(parseToNumber(groupedBywarningLogTotalTemp[ig]?.valueDay))} for contract point ${groupedBywarningLogTotalTemp[ig]?.contractPoint} and gas day ${startDateEx}`,
        );
      }
    }
