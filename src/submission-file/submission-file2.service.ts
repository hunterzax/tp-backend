...
import { getTodayEnd, getTodayEndAdd7, getTodayEndDDMMYYYYAdd7, getTodayEndDDMMYYYYDfaultAdd7, getTodayNow, getTodayNowAdd7, getTodayNowDDMMYYYYDfault, getTodayNowDDMMYYYYDfaultAdd7, getTodayStart, getTodayStartAdd7, getTodayStartDDMMYYYYAdd7, getTodayStartDDMMYYYYDfaultAdd7 } from 'src/common/utils/date.util';
...
@Injectable()
export class SubmissionFileService2 {
  constructor(
  ...
  validateDataDaily(sheetData: any) {
    ...
    for (let i = 14; i <= 37; i++) {
      if (sheetData[i.toString()] !== (i - 13).toString()) {
        console.log(`validate template fail: ${sheetData[i.toString()]} == ${(i - 13).toString()}`)
  ...
  validateDataWeekly(sheetData: any, startDateExConv: any): boolean {
    ...
    for (let i = 14; i <= 20; i++) {
      const expectedDate = dayjs(startDateExConv).add(i - 14, 'day').format('DD/MM/YYYY');
      const actualDate = sheetData[i.toString()];

      console.log(`Checking key ${i}: expected ${expectedDate}, found ${actualDate}`);
  ...
  findMatchingKeyMMYYYY(startDateExConv: any, headerEntry: any): string | null {
    ...
    for (const date in headerEntry) {
      ...
      console.log(`Checking date: ${date} (Month: ${currentMonth}, Year: ${currentYear})`);
  ...
  async uploadFile(
    ...
    const sheet1Rows: any[] = Array.isArray(sheet1?.data) ? sheet1.data : [];
    const sheet2Rows: any[] = Array.isArray(sheet2?.data) ? sheet2.data : [];
    const sheet3Rows: any[] = Array.isArray(sheet3?.data) ? sheet3.data : [];
    const sheet1HeaderRow: any[] = Array.isArray(sheet1Rows[0]) ? sheet1Rows[0] : [];
    const sheet1InfoRow: any[] = Array.isArray(sheet1Rows[1]) ? sheet1Rows[1] : [];
    const sheet1DateRow: any[] = Array.isArray(sheet1Rows[2]) ? sheet1Rows[2] : [];

    // ===== HEADER VALIDATION =====
    // Validate required headers in the first row: ['SHIPPER ID', 'CONTRACT CODE', 'START DATE']
    if (sheet1HeaderRow[0] !== 'SHIPPER ID') {
      throw new HttpException(
        {
          status: HttpStatus.BAD_REQUEST,
    ...
    if (sheet1HeaderRow[1] !== 'CONTRACT CODE') {
      throw new HttpException(
        {
          status: HttpStatus.BAD_REQUEST,
    ...
    if (sheet1HeaderRow[2] !== 'START DATE') {
      throw new HttpException(
        {
          status: HttpStatus.BAD_REQUEST,
    ...
    const shipper = await this.prisma.group.findFirst({
      where: {
        id_name: sheet1InfoRow[0], // Shipper ID from file
        status: true, // Must be active
        AND: [
          {
            start_date: {
              lte: todayEnd, // start_date must be before or equal to today end
            },
          },
          {
            OR: [
    ...
    const contractCodeName = await this.prisma.contract_code.findFirst({
      where: {
        contract_code: sheet1InfoRow[1], // Contract code from file
        AND: [
          {
            contract_start_date: {
    ...
    console.log("Contract Code Name Data : ", contractCodeName);
    ...
    const startDateEx = sheet1InfoRow[2]

    // ===== SHIPPER STATUS VALIDATION =====
    ...
    // ===== CONTRACT CODE STATUS VALIDATION =====
    ...
    // ===== SHIPPER-CONTRACT RELATIONSHIP VALIDATION =====
    ...
    if (gAuserType?.user_type_id === 3) {
      if (gAuserType?.id_name !== sheet1InfoRow[0]) {
        throw new HttpException(
          {
            status: HttpStatus.BAD_REQUEST,
            error: 'Shipper is incorrect', // https://app.clickup.com/t/86etzcgux
          },
          HttpStatus.BAD_REQUEST,
        );
      }
    ...
    // ===== CONTRACT CODE PRESENCE VALIDATION =====
    // Check if contract code field is not empty
    console.log('sheet1.data[1] : ', sheet1InfoRow);
    if (!sheet1InfoRow[1]) {
      throw new HttpException(
        {
          status: HttpStatus.BAD_REQUEST,
    ...
    // const checkTemplate = await this.prisma.$queryRaw`
    //   SELECT * FROM upload_template_for_shipper
    //   WHERE 
    //   group_id = 62 AND
    //   contract_code_id = 19 AND
    ...
    const contractCode = await this.prisma.contract_code.findFirst({
      where: {
        ...
        OR: [
          ...
          {
            ref_contract_code_by_main_id: Number(contract_code_id), // Include referenced contracts
          }
        ]
      },
      include: {
        ...
        booking_version: {
          include: {
            booking_full_json: true, // Include booking full JSON data
            booking_row_json: true,  // Include booking row JSON data
          },
          take: 1,
          orderBy: { id: "desc" } // Get latest booking version
        },
      },
      take: 1,
      orderBy: { id: "desc" }
    ...
    // const conceptPointIn = await this.prisma.concept_point.findMany({
    //   where: {
    //     // AND: [
    //     //   {
    //     //     start_date: {
    //     //       // lte: todayEnd, // start_date ต้องก่อนหรือเท่ากับสิ้นสุดวันนี้
    //     //       lte: getTodayEndDDMMYYYYAdd7(startDateEx).toDate(), // start_date ต้องก่อนหรือเท่ากับสิ้นสุดวันนี้
    //     //     },
    //     //   },
    //     //   {
    //     //     OR: [
    ...
    //     //       { end_date: { gte: getTodayStartDDMMYYYYAdd7(todayStart).toDate() } }, // ถ้า end_date ไม่เป็น null ต้องหลังหรือเท่ากับเริ่มต้นวันนี้
    //     //     ],
    //     //   },
    //     // ],
    //   },
    ...
    if (!!checkType && !!sheet2) {
      if (
        contractCode?.contract_code !== sheet1InfoRow[1] &&
        contractCode?.group?.id_name !== sheet1InfoRow[0]
      ) {
        throw new HttpException(
          {
            status: HttpStatus.BAD_REQUEST,
      ...
      } else if (contractCode?.contract_code !== sheet1InfoRow[1]) {
        throw new HttpException(
          {
            status: HttpStatus.BAD_REQUEST,
      ...
      } else if (contractCode?.group?.id_name !== sheet1InfoRow[0]) {
        throw new HttpException(
          {
            status: HttpStatus.BAD_REQUEST,
        ...
        const isEqual = headNom.every(
          (val, index) => val === sheet1DateRow[index],
        );
        ...
        // Validate that Quality sheet header matches expected format
        if (!isEqualSheet2) {
          console.log('5');
          throw new HttpException(
            {
        ...
        if (checkType === 'Daily Nomination') {
          ...
          // ===== DAILY HEADER VALIDATION =====
          // Validate that daily nomination header has correct hour format (1-24)
          const ckDateHead = this.validateDataDaily(sheet1DateRow)
          if (!ckDateHead) {
            throw new HttpException(
              {
                status: HttpStatus.BAD_REQUEST,
          ...
          sheet1 = {
            ...
            data: [
              [],
              this.uploadTemplateForShipperService.objToArr(sheet1HeaderRow),
              this.uploadTemplateForShipperService.objToArr(sheet1InfoRow),
              [
                ...(this.uploadTemplateForShipperService.objToArr(sheet1DateRow)),
                // ...daily,
              ],
              ...sheet1Rows
                .slice(3)
            ...
            data: [
              ...
              [
                ...this.uploadTemplateForShipperService.truncateArrayHeadSheet2(
                  this.uploadTemplateForShipperService.objToArr(sheet2Rows[0] ?? []),
                ),
              ],
              ...sheet2Rows
                .slice(1)
                .map((e: any) =>
                  this.uploadTemplateForShipperService.truncateArrayHeadSheet2(this.uploadTemplateForShipperService.objToArr(e)),
                ),
            ],
          };
          sheet3 = { ...sheet3, data: headNomSheet3 };

          const sheet1DataRows: any[] = Array.isArray(sheet1?.data) ? sheet1.data : [];
          for (let i = 0; i < sheet1DataRows.length; i++) {
            const dataRow: any[] = Array.isArray(sheet1DataRows[i]) ? sheet1DataRows[i] : [];
            const zoneCk = dataRow?.[0] || null
            const supplyDemandCk = dataRow?.[1] || null
            const areaCk = dataRow?.[2] || null
            const pointIdCk = dataRow?.[3] || null
            const wHvCk = dataRow?.[4] || null
            const parkUnparkInstructedFlowsCk = dataRow?.[5] || null
            const typeCk = dataRow?.[6] || null
            const areaCodeCk = dataRow?.[7] || null
            const subareaCodeCk = dataRow?.[8] || null
            const unitCk = dataRow?.[9] || null
            const entryExitCk = dataRow?.[10] || null
            const wiCk = dataRow?.[11] || null
            const hvCk = dataRow?.[12] || null
            const sgCk = dataRow?.[13] || null
            const hr1Ck = dataRow?.[14] || null
            const hr2Ck = dataRow?.[15] || null
            const hr3Ck = dataRow?.[16] || null
            const hr4Ck = dataRow?.[17] || null
            const hr5Ck = dataRow?.[18] || null
            const hr6Ck = dataRow?.[19] || null
            const hr7Ck = dataRow?.[20] || null
            const hr8Ck = dataRow?.[21] || null
            const hr9Ck = dataRow?.[22] || null
            const hr10Ck = dataRow?.[23] || null
            const hr11Ck = dataRow?.[24] || null
            const hr12Ck = dataRow?.[25] || null
            const hr13Ck = dataRow?.[26] || null
            const hr14Ck = dataRow?.[27] || null
            const hr15Ck = dataRow?.[28] || null
            const hr16Ck = dataRow?.[29] || null
            const hr17Ck = dataRow?.[30] || null
            const hr18Ck = dataRow?.[31] || null
            const hr19Ck = dataRow?.[32] || null
            const hr20Ck = dataRow?.[33] || null
            const hr21Ck = dataRow?.[34] || null
            const hr22Ck = dataRow?.[35] || null
            const hr23Ck = dataRow?.[36] || null
            const hr24Ck = dataRow?.[37] || null
            const totalCk = dataRow?.[38] || null
            // console.log('i : ', i);
            ...
            if (i > 3) {
              ...
              if (typeCk === "NONTPA") {
                const checkNonTPA = nonTpa.find((fn: any) => { return fn?.non_tpa_point_name === pointIdCk })
                if (checkNonTPA) {
                  if (checkNonTPA?.nomination_point) {
                    ...
                    for (let ifb = 0; ifb < (contractCode?.booking_version?.[0]?.booking_row_json || []).length; ifb++) {
                      const findPoint = checkNonTPA?.nomination_point?.contract_point_list.find((inb: any) => { return inb?.contract_point === contractCode?.booking_version?.[0]?.booking_row_json?.[ifb]?.contract_point })
                      if (findPoint) {
                    ...
                    if (checkNom) {
                      ...
                      throw new HttpException(
                        {
                          status: HttpStatus.FORBIDDEN,
                          // error: `${dataRow?.[3]} is activated for ${startDateEx} Click to continune`,
                          // error: `${checkNonTPA?.nomination_point?.nomination_point || "Nomination Point"} is not found in file for ${dataRow?.[3]}`,
                  ...
                  throw new HttpException(
                    {
                      status: HttpStatus.FORBIDDEN,
                      // error: `${checkNonTPA?.nomination_point?.nomination_point || "Nomination Point"} is not found in file for ${dataRow?.[3]}`,
                      error: `${dataRow?.[3]} is  not activated on Gas Day in the file.`,
                    },
                    HttpStatus.FORBIDDEN,
                  );
                }
                ...
                if ((areaCk === checkNominationPoint?.area?.name) && zoneCk === checkNominationPoint?.zone?.name && supdemCk === checkNominationPoint?.entry_exit?.name) {
                  let checkNom = false
                  for (let ifb = 0; ifb < (contractCode?.booking_version?.[0]?.booking_row_json || []).length; ifb++) {
                    const findPoint = checkNominationPoint?.contract_point_list.find((inb: any) => { return inb?.contract_point === contractCode?.booking_version?.[0]?.booking_row_json?.[ifb]?.contract_point })
                  ...
                  if (checkNom) {
                    ...
                    console.log('contractCode?.booking_version?.[0]?.booking_row_json : ', contractCode?.booking_version?.[0]?.booking_row_json);
                    console.log('checkNominationPoint?.area?.name : ', checkNominationPoint?.area?.name);
                    console.log('checkNominationPoint?.zone?.name : ', checkNominationPoint?.zone?.name);
                    console.log('checkNominationPoint?.entry_exit?.name : ', checkNominationPoint?.entry_exit?.name);
                    console.log('checkNominationPoint?.contract_point_list : ', checkNominationPoint?.contract_point_list);
                    ...
                    throw new HttpException(
                      {
                        status: HttpStatus.FORBIDDEN,
                        // error: `${dataRow?.[3]} is activated for ${startDateEx} Click to continune`,
                        // error: `${areaCk}, ${zoneCk}, or ${supdemCk} for ${dataRow?.[3]}  is incorrected`,
                        error: `Nomination Point is incorrect.`, // https://app.clickup.com/t/86etzcgzh
                      },
                      HttpStatus.FORBIDDEN,
                    );
                ...
                if (!!dataRow?.[0] && dataRow?.[3]) {
                  ...
                  if (!findConcept) {
                    ...
                    throw new HttpException(
                      {
                        status: HttpStatus.FORBIDDEN,
                        // error: `${dataRow?.[3]} is incorrected`,
                        // error: `${dataRow?.[3]} is activated for ${startDateEx} Click to continune`,
                    ...
                    throw new HttpException(
                      {
                        ...
                        // error: `${dataRow?.[3]} is incorrected`,
                        // error: `${dataRow?.[3]} is activated for ${startDateEx} Click to continune`,
                        error: `No permission for this Concept Point ${dataRow?.[3]} ,Please set the limit first.`, // https://app.clickup.com/t/86etzch1z // https://app.clickup.com/t/86etzcgza
                      },
                      HttpStatus.FORBIDDEN,
                    );
                  }
          ...
          const sheet2DataRowsDaily: any[] = Array.isArray(sheet2?.data) ? sheet2.data : [];
          for (let i = 0; i < sheet2DataRowsDaily.length; i++) {
            const zoneCk = sheet2DataRowsDaily[i]?.[0] || null
            const pointIdCk = sheet2DataRowsDaily[i]?.[1] || null
          ...
          const sheet1DataRowsWeekly: any[] = Array.isArray(sheet1?.data) ? sheet1.data : [];
          for (let i = 0; i < sheet1DataRowsWeekly.length; i++) {
            const dataRow: any[] = Array.isArray(sheet1DataRowsWeekly[i]) ? sheet1DataRowsWeekly[i] : [];
            const zoneCk = dataRow?.[0] || null
            const supplyDemandCk = dataRow?.[1] || null
            const areaCk = dataRow?.[2] || null
            const pointIdCk = dataRow?.[3] || null
            ...
            const hvCk = dataRow?.[12] || null
            const sgCk = dataRow?.[13] || null
            const day1Ck = dataRow?.[14] || null
            const day2Ck = dataRow?.[15] || null
            const day3Ck = dataRow?.[16] || null
            const day4Ck = dataRow?.[17] || null
            const day5Ck = dataRow?.[18] || null
            const day6Ck = dataRow?.[19] || null
            const day7Ck = dataRow?.[20] || null
          ...
          const sheet2DataRowsWeekly: any[] = Array.isArray(sheet2?.data) ? sheet2.data : [];
          for (let i = 0; i < sheet2DataRowsWeekly.length; i++) {
            const zoneCk = sheet2DataRowsWeekly[i]?.[0] || null
            const pointIdCk = sheet2DataRowsWeekly[i]?.[1] || null
    ...
    const bookingFullJsonRaw = contractCode?.booking_version?.[0]?.booking_full_json?.[0]?.data_temp;
    if (!bookingFullJsonRaw) {
      throw new HttpException(
        {
          status: HttpStatus.BAD_REQUEST,
    ...
    if (headerExitCDBMMBTUD && typeof headerExitCDBMMBTUD === 'object') {
    ...
    if (headerEntryCDBMMBTUH && typeof headerEntryCDBMMBTUH === 'object') {
      delete headerEntryCDBMMBTUH['key']
    }
    const headerExitCDBMMBTUH = bookingFullJson?.headerExit?.['Maximum Hour Booking (MMBTU/h)']
    if (headerExitCDBMMBTUH && typeof headerExitCDBMMBTUH === 'object') {
      delete headerExitCDBMMBTUH['key']
    }

    const entryValue: any[] = Array.isArray(bookingFullJson?.entryValue) ? bookingFullJson.entryValue : [];
    ...
    const checkEmtry = getsValue?.map((re: any) => re?.row?.map((rer: any) => false))
    ...
    // sheet 2 check
    const sheet2DataRows: any[] = Array.isArray(sheet2?.data) ? sheet2.data : [];
    const indexSheetLastValue = sheet2DataRows.findIndex((row: any) => Array.isArray(row) && row.includes("*"));
    const fullShee2Data = []
    for (let i = 0; i < sheet2DataRows.length; i++) {
      if (i > 1 && i < (indexSheetLastValue)) {
        // console.log('--> : ', sheet2?.data?.[i]);
        fullShee2Data.push(sheet2DataRows[i])
        const zone = sheet2DataRows[i]?.[0]
        const contractPoint = sheet2DataRows[i]?.[1]
    ...
    const nominationRowJson = [
      ...(nominationFullJson?.typeDoc?.columnPointId.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 1, } })),
      ...(nominationFullJson?.typeDoc?.columnPointIdConcept.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 2, } })),
      ...(nominationFullJson?.typeDoc?.columnType.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 3, } })),
      ...(nominationFullJson?.typeDoc?.columnParkUnparkinstructedFlows.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 4, } })),
      ...(nominationFullJson?.typeDoc?.columnWHV.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 5, } })),
      ...(nominationFullJson?.typeDoc?.columnOther.map((e: any) => { return { zone_text: e?.row?.["0"] || null, area_text: e?.row?.["2"] || null, entry_exit_id: e?.row?.["1"] === "Supply" ? 1 : e?.row?.["1"] === "Demand" ? 2 : null, data: e?.row, old_index: e?.ix, type: 6, } })),
    ]
    ...
    // console.log('nominationCount : ', nominationCount);
    // return nomination_code

    const warningAll = [...sheet1Quality, ...sheet2Quality, ...warningLogHr, ...warningLogDayWeek, ...warningLogDay]

    // ===== WARNING HANDLING =====
    // Check if there are any warnings that require user interaction
    const hasWarnings = warningAll.length > 0;
    const hasOveruseQuantity = overuseQuantity || false;
    const hasOverMaximumHourCapacity = overMaximumHourCapacityRight || false;
    const hasQualityIssues = sheet1Quality.length > 0 || sheet2Quality.length > 0;

    // If there are warnings, include them in the response for user decision
    if (hasWarnings) {
      console.log('⚠️ Warnings detected that require user attention:');
      console.log('- Overuse Quantity:', hasOveruseQuantity);
      console.log('- Over Maximum Hour Capacity:', hasOverMaximumHourCapacity);
      console.log('- Quality Issues:', hasQualityIssues);
      console.log('- Total Warnings:', warningAll.length);
    }
    ...
    const finalData = {
      ...
      overuseQuantity: warningLogDayWeek.length > 0 || warningLogDay.length > 0 ? true : false,
      overMaximumHourCapacityRight: warningLogHr.length > 0 || warningLogDayWeek.length > 0 ? true : false,
      warningLogHr: warningLogHr,
      warningLogDay: warningLogDay,
      warningLogDayWeek: warningLogDayWeek,
      warningAll,
      informationData,
      // ===== WARNING SUMMARY FOR USER INTERACTION =====
      warningSummary: {
        hasWarnings,
        hasOveruseQuantity,
        hasOverMaximumHourCapacity,
        hasQualityIssues,
        totalWarnings: warningAll.length,
        requiresUserDecision: hasWarnings, // Flag to indicate if user needs to make a decision
        warningTypes: {
          overuseQuantity: hasOveruseQuantity,
          overMaximumHourCapacity: hasOverMaximumHourCapacity,
    ...
    // nominationFullJson?.typeDoc?.columnType
    // nomination_type_id
    // non_tpa_point

    if (nomination_type_id === 1) {
      console.log('nominationFullJson : ', nominationFullJson);
      console.log('nominationFullJson?.typeDoc?.columnPointId : ', nominationFullJson?.typeDoc?.columnPointId);
    ...
    // if (checkVersion) {
    //   // check gasday nowdate
    //   const isFuture = startDateExConv.isBefore(getTodayStartAdd7(), 'day');
    //   const isSame = startDateExConv.isSame(getTodayStartAdd7(), 'day');
    //   console.log(startDateExConv);
    //   console.log(getTodayStartAdd7());
    ...
    //           .add(nominationDeadlineReceptionOfRenomination?.before_gas_day, 'day')
    //           .set('hour', nominationDeadlineReceptionOfRenomination?.hour)
    //           .set('minute', nominationDeadlineReceptionOfRenomination?.minute)
    //           .startOf('minute');

    //         const renomCurrentDeadline = getTodayNowAdd7()
    //           .set('hour', nominationDeadlineReceptionOfRenomination?.hour)
    //           .set('minute', nominationDeadlineReceptionOfRenomination?.minute)
    //           .startOf('minute');

    //         if (startDateExConv.isBefore(renomAllowedDate) || getTodayNow().isAfter(renomCurrentDeadline)) {
    //           checkVersion = null
    //         }
    //       }
    //     } else {
    //       // weekly
    //       const allowedDate = getTodayNow()
    //         .add(nominationDeadlineSubmission?.before_gas_day, 'week')
    //         .set('hour', nominationDeadlineSubmission?.hour)
    //         .set('minute', nominationDeadlineSubmission?.minute)
    //         .startOf('minute');

    //       // ✅ เวลาปัจจุบันสำหรับตรวจสอบ (เวลา 18:00)
    //       const currentDeadline = getTodayNowAdd7().set('hour', nominationDeadlineSubmission?.hour)
    //         .set('minute', nominationDeadlineSubmission?.minute)
    //         .startOf('minute');

    //       console.log('Allowed Date:', allowedDate.format('DD/MM/YYYY HH:mm'));
    //       console.log('Current Deadline:', currentDeadline.format('DD/MM/YYYY HH:mm'));
    //       console.log('Start Date:', startDateExConv.format('DD/MM/YYYY HH:mm'));

    //       // ✅ ตรวจสอบว่า startDateExConv ต้องเป็นวันอาทิตย์
    //       const isSunday = startDateExConv.day() === 0; // 0 = Sunday

    //       // ✅ เงื่อนไขตรวจสอบ Submission
    //       if (
    //         startDateExConv.isBefore(allowedDate) ||
    //         getTodayNow().isAfter(currentDeadline) ||
    //         !isSunday // ต้องเป็นวันอาทิตย์เท่านั้น
    //       ) {
    //         // ✅ ตรวจสอบใหม่โดยใช้ Renomination
    //         const renomAllowedDate = getTodayNow()
    //           .add(nominationDeadlineReceptionOfRenomination?.before_gas_day, 'week')
    //           .set('hour', nominationDeadlineReceptionOfRenomination?.hour)
    //           .set('minute', nominationDeadlineReceptionOfRenomination?.minute)
    //           .startOf('minute');

    //         const renomCurrentDeadline = getTodayNowAdd7().set('hour', nominationDeadlineReceptionOfRenomination?.hour)
    //           .set('minute', nominationDeadlineReceptionOfRenomination?.minute)
    //           .startOf('minute');

    //         const isRenomSunday = startDateExConv.day() === 0; // ตรวจสอบวันอาทิตย์สำหรับ Renomination
    //         // ✅ เงื่อนไขตรวจสอบสำหรับ Renomination
    //         if (
    //           startDateExConv.isBefore(renomAllowedDate) ||
    //           getTodayNow().isAfter(renomCurrentDeadline) ||
    //           !isRenomSunday // ต้องเป็นวันอาทิตย์เท่านั้น
    //         ) {
    //           checkVersion = null
    //         }
    //       }
    ...
    if (checkVersion) {
      ...
      const excelStart = dayjs(nominationFullJson?.shiperInfo?.['2']["START DATE"], "DD/MM/YYYY").format("YYYY-MM-DD")
      ...
      if (comment) {
        await this.queryShipperNominationFileService.comments({ reasons: false, comment: comment, query_shipper_nomination_file_id: Number(checkVersion?.id) }, userId)
      }
      ...
      // ===== SAVE QUALITY SHEET DATA =====
      // Create nomination full JSON sheet2 (Quality sheet data)
      const fullJson2 = await this.prisma.nomination_full_json_sheet2.create({
        data: {
          data_temp: JSON.stringify(nominationFullJsonSheet2), // Quality sheet data as JSON
          nomination_version: {
            connect: {
              id: nominationVersion?.id, // Link to nomination version
            },
          },
          flag_use: true, // Mark as active
          create_date_num: newDate.unix(), // Unix timestamp
          create_date: newDate.toDate(), // Date object
          create_by_account: {
            connect: {
              id: Number(userId), // Link to user who created
            },
          },
        },
      })
      ...
      // ===== SAVE ROW DATA =====
      // Create nomination row JSON data (individual row data)
      const rowJson = await this.prisma.nomination_row_json.createMany({
        data: (nominationRowJson || []).map((e: any) => {
          return {
            nomination_version_id: nominationVersion?.id, // Link to nomination version
            flag_use: true, // Mark as active
            zone_text: e?.zone_text, // Zone information
            area_text: e?.area_text, // Area information
            entry_exit_id: e?.entry_exit_id, // Entry/exit point ID
            query_shipper_nomination_type_id: e?.type, // Nomination type
            data_temp: JSON.stringify(e?.data), // Row data as JSON
            old_index: e?.old_index, // Original row index
            create_date_num: newDate.unix(), // Unix timestamp
            create_date: newDate.toDate(), // Date object
            create_by: Number(userId), // User who created
          }
        })
      })

      // ===== SAVE WARNING MESSAGES =====
      // Create submission comment/warning messages
      const submissionFile = await this.prisma.submission_comment_query_shipper_nomination_file.createMany({
        data: (warningAll || []).map((e: any) => {
          return {
            remark: e, // Warning message
            query_shipper_nomination_file_id: Number(queryShipperNominationFile?.id), // Link to submission file
            create_date_num: newDate.unix(), // Unix timestamp
            create_date: newDate.toDate(), // Date object
            create_by: Number(userId), // User who created
          }
      ...
      // ===== SAVE FILE URL =====
      // Create file URL record for uploaded file
      const queryShipperNominationFileUrl = await this.prisma.query_shipper_nomination_file_url.create({
        data: {
          url: finalData?.dataInfo?.files, // File URL/path
          query_shipper_nomination_file: {
            connect: {
              id: queryShipperNominationFile?.id, // Link to submission file
            },
          },
          nomination_version: {
            connect: {
          ...
          query_shipper_nomination_status: {
            connect: {
              id: queryShipperNominationFile?.query_shipper_nomination_status_id, // Link to nomination status
            },
          },
          create_date_num: newDate.unix(), // Unix timestamp
          create_date: newDate.toDate(), // Date object
